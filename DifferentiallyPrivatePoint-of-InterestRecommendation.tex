...
We investigate two attacking models which may reveal the private information of the user’s location and friend ties in the previous section so as to put forward the corresponding privacy-preserving algorithms for them now and then evaluate the utility and performance of our approaches in detail. We also build them two methods into the recommender framework as our novel idea to return recommendations enjoying privacy guarantees followed by strict theoretical analysis.

\subsection{GLP: Geographical location privacy-preserving algorithm}\label{exp:Location PPA}
We will propose the GLP algorithm to preserve location privacy for the geographical attacking model in Section \ref{exp:AttackingModels}. Lu et al.~\cite{HLU2008} proposed a general preserving method called $\left\langle k, s\right\rangle$-privacy which is basing on the generator of virtual nodes to blurred geographical location. They proposed two dummy methods called CirDummy and GridDummy to realize $\left\langle k, s\right\rangle$-Privacy, where CirDummy is dedicated to generating $k-1$ additional nodes in a virtual circle which contains the user's real position with a random center and an area of $k\cdot s$ and GridDummy is meant to produce a big virtual square which consists of $k$ small squares with an area of $s$ and make the user's real position be a point of any small square. We notice many deficiencies of this algorithm. Note that $\left\langle k, s\right\rangle$-privacy algorithm needs receive two parameters $k$ and $s$ and fixed $k$ and $s$ can not be adapted to all regions, i.e., it is not taken into account that population densities vary widely from place to place. For example, there is a large population density in the lively commercial street and large parameters will not significantly increase privacy-preserving degree but will significantly reduce the availability of the valuable data. Conversely, in the rural areas or other areas with a relatively sparse population, unchangeable parameters seem too small. At the same time, due to sparse population and flat terrain, attackers can even easily catch the user in his current fuzzy area quickly, which exposes the user's location privacy. On the other hand, in the design of  $\left\langle k, s\right\rangle$-privacy, there is no quantification of the actual degree of privacy preservation, so that the user can not understand how much private it is if he took different privacy parameters. Last but not least, the privacy parameters may be too large to many of the service providers, so it has basically lost the value of the data because these large data can not be effectively used. These problems make recommendation service providers lose the opportunity to improve the user experience and give much privacy guarantees.

\subsubsection{$\langle r, h\rangle$-Privacy}
We improve the algorithm for the defects and shortcomings of $\left\langle k, s\right\rangle$-privacy, and put forward $\langle r, h\rangle$-privacy algorithm, and use Shannon information entropy to carry out theoretical evaluation. $\langle r, h\rangle$-privacy inherits the idea of $\langle k, s\rangle$-privacy but we do not generate virtual nodes any more. We directly make the user position blur into a virtual circle, and the precise position may exist anywhere in the circle. In this ambiguous mode, the probability of the attacker inferring the user's real position approaches $0$. In such a low probability case, the attacker may also have a kind of violent way to find the user and the $\langle r, h\rangle$-privacy algorithm is also able to adjust the radius of the circle based on the local population, allowing the user to have enough time to leave without being found. It also assumes that an attacker can obtain all the information from the server, as the $\langle k, s\rangle$-privacy. If a user wants to send a position to the server, a fuzzy one will be sent in our settings.
...
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
  ...
  \State $R_i \leftarrow R(h_i)$ \text{// the radius are based on $h_{i}$ in Eq.(\ref{equ:VirtualCR})}
         \If{$h_{i} < 3$}
           ...
           \State datermine $o^{\prime}$ with a vector ($l,\theta$)
         \Else
  ...
  \State
  \State \Return{$o_i^{\prime}$ and $R_i$}
  \EndFunction
  \end{algorithmic}
  \end{algorithm}
\hrule
...
There are several ways to obtain the virtual circle radius when calculating the total number of a POI records as a parameter. And to be simple and effective, we choose the linear function to calculate:
\begin{equation}\label{equ:VirtualCR}
R_i=-\frac{R_{max}-R_{min}}{H_{max}}\cdot h_i+R_{max}
\end{equation}
where $R_{max}$ and $R_{min}$ respectively represent the upper and lower bounds of the virtual circle radius, and $H_{max}$ represents the maximum number of historical check-in numbers in all POIs($H_{max}=max\{h(POI_1),h(POI_2),\dots,h(POI_n)\}$). Last but not least , $R_{min}$ of virtual circle radius is set to prevent some hotspot POI from losing the privacy guarantee as the virtual circle radius is too small as the number of user checkouts increases.
\par In the $\langle r,h\rangle$-privacy algorithm, the total records $H_{max}$, the upper and lower bounds of the virtual circle radius $R_{max}$ and $R_{min}$, need to be further determined. $H_{max}$, such as Foursquare data ,can reach more than 10,000, and overview all the POI history records, only a small amount of POI can reach 10,000 or so, the vast majority of the POI history check number even less than 10. So it is more important to determine the upper bound of the radius than the lower bound. The empirical value of $R_{max}$ is 200 meter so the maximum area of the virtual area is $S_{circle}=\pi\cdot R_{max}^{2} \approx 12500m^2 $ , only the $1/8$ of maximum privacy area in $\langle k, s\rangle $-privacy algorithm.
\par $\left\langle r, h\right\rangle$-Privacy will also face the situation of failure. For example, some POI history check numbers are only 1, and in this case the area of virtual circle reaches maximum. Also the center point $POI_i$ would move to a new center $o^{\prime}$ with the vector of angle $\theta(\theta\in[0,2\pi])$, length $l(l \in(0,R))$, however, it is possible there will be a small length $l$. Under this certain situation, attackers can quickly locate the precise position of the POI and catch the target user. Hence, in this case, we let length $l\in[R/2,R]$ to ensure that the virtual circle center will not be close to the user's real position.
...
\par When the algorithm is added to the POI recommendation system, the distance between any two POIs is required according to Eq.(\ref{eq:LW}) and Eq.(\ref{eq:PRLJ}) in Section \ref{sec:RelatedWork} when calculating the geographical location factor. In this paper, the distance $z_{i,j}=Dist(POI_i,POI_j)$ between two private POIs is defined as the distance between their virtual circles so as to facilitate the calculation. According to the Theorem \ref{theorem:Dd}, the probability density function of $z_{i,j}$ is:
...
\end{equation}
\indent And the POI was blurred into a virtual circle, resulting in the distance uncertainty between POIs. Hence, the differential entropy of the random variable $z_{i,j}$ can be calculated as
...
In the social networking recommender system, each user's friends are potential attackers. They can derive the user's privacy check-in information with a large probability from the POI recommendation results. In Examples \ref{e.g.1} and \ref{e.g.2}, we have assumed that the attacker knows the recommendation algorithm of the recommender system, but the setting of each parameter in the algorithm is not achieved by attackers. For example, in Section \ref{sec:Background}, the predicted probability of the user for the POI is composed of three parts: user similarity , friend relationship and location which are combined in a linear function, where the linear parameters are $1-\alpha-\beta$ , $\alpha$ and $\beta$, $0\leqslant \alpha+\beta\leqslant1$. In Example \ref{e.g.1}, the attacker Anna and the normal user Bron often go out and visit POIs together. Therefore, in the recommender system, Anna and Bron have higher user similarity $S_{A,B}^u$ , and they are friends, so their friend relationship probability $S_{A,B}^s$ is also high. In the top-$K$ results returned by the recommender system to Anna, some abnormal points appear. These abnormal points are far away from the current location of hers, i.e, the location probability $S_{A,B}^g$ is very low. Anna understands the recommendation algorithm so she infers that the POI in the recommended result must be the highest predicted check-in probability for her. Hence, in the case of that three linear parameters $1-\alpha-\beta$ ,$\alpha$ and $\beta$ are constant, the user similarity factor $S_{A,B}^u$, and friend relationship factor $S_{A,B}^s$, must be very high, so Anna can believe with a high probability that this anomaly is because of her best friend Bron who has a similar preference.
This section we will propose the FRP algorithm based on differential privacy for the friend relationship attacking model in Section \ref{exp:AttackingModels}.
...
\par FRP algorithm is based on social network and it can prevent the potential attack. The attacker may infer the target user's privacy information with a high probability if recommendation probabilities of the user similarity, friend relationship and location have a large difference in the POI recommender system. Our algorithm can be simply understood that it can add enough noise to the friend relationship factor to make it become more smooth, so that the attacker can not infer that his friend would have anything with the returned results from the system and the attacker can not obtain any related private information.
...
\end{corollary}
\par Let $Pr\{C_{B,j}=1|top-K POI\}$ represent that attackers can infer the probability of user B checking in the $POI_j$ on the premise of system returning top-K POIs. The inference probability will not be particularly high if we add FRP in the recommender system, even though the attackers know the working process of the system. Although there are a number of basic definitions of privacy in the existing literatures, most definitions are too broad and have a drawback of no quantitative indicators, so in this article, we choose $\epsilon$ - differential privacy to build into FRP shown in Algorithm \ref{alg_frppA}. The algorithm first randomly deletes one user $u_i$'s one friend relationship link from the database $D$, i.e., they are no longer friends, and then generates a new neighbor database $D^{\prime}$ (lines 3-4). Second, we obtain the $L1$-vector norm of the two weight vectors, that is, the global sensitivity of the query function (line 5). The scale parameter of Laplacian noise can be obtained from the sensitivity of the query function (line 6) and the positional parameter is $0$. According to this, we can get the complete Laplace distribution and add Laplacian noise to the original friend relationship factor vector finally. We can see that the friend relationship factor become smoother and closer, and it will not exist the situation that the friend relationship weights of some users are particularly high.
\\
\hrule
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
 ...
 % \Function{\textit{Social Differential Privacy}}{the privacy parameter $\epsilon$}
  \For {each $u_i$ with user $u_k$'s $SI_{i,k}^{\prime}$ }
  \State database $D$ denotes all $u_i$’s relationship
  \State $D^’$ denotes delete one of $u_i$’s relationship, $D^{\prime}\leftarrow D-\{u_i,u_{random}\}$
  \State sensitivity $\Delta f\leftarrow max_{D,D^{\prime}}||SI_{i,k}(D)-SI_{i,k}(D^{\prime})||_1$
  \State scale factor $sf\leftarrow\Delta f /\epsilon$
  \State $\forall SI_{i,k}^{\prime}\leftarrow SI_{i,k}+Laplace(0,sf) $
  \EndFor
...
2^{SI_i^{\prime})-H(SI_i)}=2^{1+\ln{(\frac{2\Delta f}{\epsilon})}}=4\times\frac{\Delta f}{\epsilon}
\end{equation}
...
\subsection{Theoretical analysis of private POI recommendation}\label{sec:analysis}
We employ shannon entropy to measure the degree of privacy guarantee. First, to define privacy precisely, we need to determine a suitable model to describe the database. In many application domains, data can be represented as data tables, e.g., medical records, financial transaction records or employee records. The data table can be regarded simply as a relational database which normally consists of several data tables. A formal definition of database is as follows,
...
As shown in Fig.\,\ref{fig:PrivateRecall5andPrecision5}, in the adjustment of the linear parameters of the private version, it can be seen that the private algorithm does not reduce the validity of the recommendation {\color{red}(???)} significantly, and both $\alpha$ and $\beta$ are exactly equal to $0.1$ when recall rate and accuracy rate get the optimal point. {\color{red} And even the privacy algorithm does not have a very large impact on the accuracy of recommendation algorithm in the extreme situation, we also conclude that the weight of user similarity is larger than friend relationship and the weight of friend relationship is greater than that of location.}
