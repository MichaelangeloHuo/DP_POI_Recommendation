...
\subsection{Outline}
The rest of the paper is organized as follows. Section \ref{sec:RelatedWork} gives some related works and Section \ref{sec:Background} reviews the POI recommendation system, differential privacy and the private attacking models. Section \ref{sec:Location PPA} shows the fuzzy geographic location algorithm. Fuzzy friend relationship privacy-preserving algorithm is proposed in Section \ref{sec:FRPPA}, followed by theoretical analysis of POI privacy recommendation algorithm in Section \ref{sec:analysis} and extensive experimental results in Section \ref{sec:experiment} to demonstrate the effectiveness of the proposed methods. Section \ref{sec:Conclusion} concludes the work and give some advises on the future work.
...
{\color{red} We focus on differential privacy with its Laplacian mechanism because we have to calculate the social relationship weight which is a numeric value between users in the recommender system. We first add Laplacian noise to the social relationship factor but not the final recommender result to smooth the weights between friend users to avoid social relationship attacks. Through this design we realize our original idea enjoying differentially private guarantees in public recommender servers.}
...
\subsubsection{Geographical location weight}
\begin{figure}[!htb]
    \centering
...
\end{equation}
Hence, we can calculate the probability $Pr[l_j\big\vert L_i ] \,(l_i\in L-L_i)$ of the POI that the user has not signed in and sort it in descending order, and then recommend top-K POIs to the user.

\subsubsection{Overall POI weights}
As discussed, we can integrate the user similarity factor, the friend relationship factor and the geo-location factor into a linear function, and calculate the probability that the user will check in a POI. Let $S_{i,j}$ be the probability that user $u_i$ will check in the POI $l_j$ and then let $S_{i,j}^u, S_{i,j}^s, S_{i,j}^g$ represent the user similarity weight, the friend relationship weight, location weight respectively, where $S_{i,j}$  is defined as
...
\cite{CDwork2006} A randomized mechanism $M_{priv}(\cdot):D\rightarrow S$ gives ($\epsilon,\delta$)-differential privacy for every set of outputs S and for two adjacent datasets of $D$ and $D^{'}$，if $M_{priv}$ satisfies
\begin{equation}\label{eq:DP}
Pr[M_{priv}(D)\in S] \leqslant e^{\epsilon}\cdot Pr[M_{priv}(D^{'})\in S] + \delta
\end{equation}
\end{definition}
...
\cite{CDwork2011} For a query $Q:D\rightarrow S$, and two adjacent datasets $D$ and $D^{'}$, the sensitivity $\Delta Q$ of query $Q$ is defined as
...
\end{equation}
\end{definition}
Laplacian mechanism uses the sensitivity of the $Q$ as a parameter and adds Laplacian noise to the output of the query result. But for non-numeric queries, differential privacy uses an exponential mechanism for noise results because Laplace mechanism failed to solve this problem.
...
\subsection{Attacking privacy model}
...
\end{example}
\par These two examples tell us two different attacking patterns basing on friendship. We will solve these problems in Section \ref{sec:FRPPA}.

%\subsection{Private Recommendation}


\section{GLP: Geographical location privacy-preserving algorithm}\label{sec:Location PPA}
We continue on the POI recommendation system in the previous chapter. Now we investigate the attacking mode which may reveal the private information of the user's position and put forward the corresponding privacy-preserving algorithm and then evaluate the utility and performance of the algorithm in detail.

\subsection{ $\langle k, s\rangle$-Privacy location privacy-preserving algorithm}

Lu et al.~\cite{HLU2008} proposed a general protected method for the blurring geographical location. The algorithm can be described as adding virtual nodes so that the query algorithm output can meet privacy requirements.

{\color{red}
\subsubsection{Basic definitions and principles}
A location-based query is represented by $Q=(pos,P)$, where $pos$ is the user's geographic location parameter, $P$ represents the user-specified predicate. This query $Q$ is called the original query. After the virtual node has been added, the original query is converted to $Q^{\prime}=(pos_1,pos_2,\dots,pos_k,P)$, where $pos_i$ contains the user's real location and  $k-1$ virtual nodes, and the original query predicate $P$ is applied to all the $k$ nodes.
\begin{definition}[Privacy Area]\label{def:PA}
\cite{HLU2008} The privacy area of a geo-private query is a convex hull of all location parameters in the query.
\end{definition}
The convex hull is the smallest convex region that covers all positions in the geographical privacy query. It is a natural and conservative guess for the opponent who knows the position of the user in all positions.
\begin{definition}[$\left\langle k, s\right\rangle$-Privacy]\label{def:ksP}
\cite{HLU2008} If a geo-location privacy query contains $k$ geographies and the privacy area in Definition \ref{def:PA} is not less than $s$, then the query satisfies $\left\langle k, s\right\rangle$-Privacy.
\end{definition}
}



\subsubsection{$\left\langle k, s\right\rangle$-privacy algorithm}
$\left\langle k, s\right\rangle$-privacy algorithm is based on the generator of virtual nodes. By adding a virtual node to confuse the real node, it converts the original query $Q$ to a geo-private query $Q^{\prime}=(pos_1,pos_2,\dots,pos_k,P)$. In the virtual node-generated algorithm, we need to consider the private area parameter $s$, so that the convex of the position parameter can cover the $k$ positions under the premise that the private area can be as close as possible to $s$ or equal to $s$.
\par \noindent \textbf{(1)CirDummy Algorithm.} All locations, including the user's physical location, are restricted in a circle with a circle of $pos^{'}$ and the radius $r$. The center of the circle is random in range $r_{min}\leqslant dist(pos,pos^{'})\leqslant r$, where dist $(pos,pos^{'})$ is the Euclidean distance between $pos$ and $pos^{'}$. Each of the two consecutive selected nodes has an angle $\theta$ for the center of the circle. All positions are distributed in such a way that all $\theta$s are equivalent. Hence the entire private area equals to the sum of areas of $k$ triangles:
\begin{equation}\label{eq:Privacy Area}
\hat{s} = \sum_{i=1}^{k}\frac{1}{2}\cdot r_i\cdot r_{i+1}\cdot \sin\theta=\frac{1}{2}\sum_{i=1}^{k}r_i\cdot r_{i+1}\cdot \sin\frac{2\pi}{k}
\end{equation}
where $r_i$ represents $dist(pos,pos^{'})$.
\\\indent To simplify Eq.(\ref{eq:Privacy Area}), we make $r_{k+1}=r_1$, and assume that the distance from geographical position to $pos^{'}$ is the same, then the hull determined by them is necessarily convex. Thus, taking into account the private area requirement $s$, we have:
\begin{equation}\label{eq:Privacy Area_2}
\hat{s}=\frac{1}{2}\cdot k \cdot r_i^2 \cdot\sin\frac{2\pi}{k}
\end{equation}
\indent According to the upper bound of the radius $r=\sqrt{(2\cdot s)/(k\cdot\sin{2\pi/k})}$, we let $r_{min}=\rho\cdot r$, where $0\textless\rho\textless1$, then:
\begin{equation}\label{eq:Privacy Area_3}
\hat{s} \geqslant \frac{1}{2}\cdot k \cdot r_{min}^2 \cdot\sin\frac{2\pi}{k}=\frac{1}{2}\cdot k \cdot (\rho\cdot r)^2 \cdot\sin\frac{2\pi}{k}=\rho^2\cdot s
\end{equation}


This indicates a lower bound of the privacy area of all the $k$ positions. The algorithm can guarantee the location privacy through proper parameters. For example, if we choose $\rho=\sqrt3 /2$, we can ensure that the resulting privacy area is not less than $3s/4$.
\par The algorithm, called CirDummy, is shown in Algorithm \ref{alg_CDA}.  The algorithm firstly calculates the upper bound of the radius $r$ of the virtual circle, and the angle $\theta$ between every consecutive position pairs(line 3). Secondly a center point of the virtual circle is randomly determined in the range of $r$ so that the distance from the circle to the pos is $[\rho\cdot r,r]$(line 4). Thirdly initialize an empty array $K$ and place the user's real address at its first element(line 5). The location of the remaining $k-1$ virtual nodes is generated as follows: the distance to the virtual center $pos^{'}$ is $[\rho\cdot r,r]$, so that they are able to fall evenly within the circle(lines 6-10). Finally, generates a random number $i_{dx}$ between $0$ and $k-1$, then returns $i_{dx}$ and $K$(lines 11-13).
\\
...
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
  \begin{algorithm}{CirDummy Algorithm}\label{alg_CDA}
  %\caption {CirDummy Algorithm}%\label{alg_CDA}
  \begin{algorithmic}[1]
  \Require User position $pos$, number $k$ , privacy area $s$ ,coefficient $\rho$
  \Ensure $K$, $i_{dx}$
  \Function{\textit{CirDummy}}{User position $pos$, number $k$ , privacy area $s$ ,coefficient $\rho$}
  \State $\theta\leftarrow 2\pi k$
  \State $r\leftarrow \sqrt{2s/(k\cdot\sin\theta)}$
  \State determine a position $pos^{'}$ at random $s.t. dist(pos,pos^{'})\in [\rho\cdot r,r]$
  \State initialize $K[0,\dots,k-1]$, $K[1]\leftarrow pos$
   % \State $   Data^{need}_{max} \gets Data^{t,w,need} _{max}$
       \For{$i = 1 \to k-1$}
             \State determine a position $p$ at random $s.t. dist(pos,pos^{'})\in [\rho\cdot r,r]$
             \State and $\angle p~pos^{'}~K[i-1]= \theta$
             \State $K[i]\leftarrow p$
      \EndFor
    \State $i_{dx}\leftarrow random(0,k-1)$
    \State switch $K[0]$ with $K[i_{dx}]$
    \State \Return{$k$ and $i_{dx}$}
    \EndFunction
   \end{algorithmic}
   \end{algorithm}
\hrule
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
\vspace{0.5cm}
\par When the coefficient $\rho$ is very close to $1$, it is possible to make all the positions close to the edge of the virtual circle and leave a lot of blank area around the circle center. The algorithm can be adapted to handle such cases. We can generate half of dummies first, and another half in the range of the distance $[0,\rho\cdot r]$ to $pos^{'}$(line 4 in Algorithm \ref{alg_CDA}).
\par\noindent\textbf{(2)GridDummy algorithm.} The grid-based dummy generation works as: first, the grid has $k$ vertices, then its area is equal to $s$, finally the user's real position is also one of its vertices, so the remaining $k-1$ vertices are virtual nodes. We describe the GridDummy algorithm in Algorithm \ref{alg_GDA}, and it is used to calculate the number of vertices $k$ in each direction first (line 2). Then, set the coordinate system according to the number of vertices and give the coordinate values of each vertex (lines 3-4). Then calculate the length $g$ of each grid based on the given area parameter $s$ and the grid coordinates (line 5). Then calculate the geographical position of each vertex and put the calculation results into the array $K$ (lines 6-13). Finally return the array $K$  and the index of the user position in the array $K$ (line 14).
\\

\hrule
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
  \begin{algorithm}{GridDummy Algorithm}\label{alg_GDA}
  \begin{algorithmic}[1]
  \Require User position $pos$, number $k$ , privacy area $s$
  \Ensure $K$, the index of $pos$ in $K$
  \Function{\textit{GridDummy}}{User position $pos$, number $k$ , privacy area $s$}
  \State $c \leftarrow \sqrt{k}$
  \State $id_x \leftarrow random(0,c-1)$
  \State $id_y \leftarrow random(0,c-1)$
  \State $g\leftarrow \sqrt{s}/(c-1)$
  \State initialize $K[0,\dots,k-1]$
   % \State $   Data^{need}_{max} \gets Data^{t,w,need} _{max}$
       \For{$i = 0 \to c-1$}
           \For{$j = 0 \to c-1$}
             \State $x \leftarrow (i-id_x)\cdot g + pos.x$
             \State $y \leftarrow (i-id_y)\cdot g + pos.y$
             \State $K[j*c+i] \leftarrow (x,y)$
           \EndFor
      \EndFor
    \State \Return{$K$ and $id_y*c+id_x$\text{//the index of $pos$ in K}}
    \EndFunction
   \end{algorithmic}
   \end{algorithm}
\hrule

\subsubsection{The shortcomings of $\left\langle k, s\right\rangle$-privacy algorithm}
In the $\left\langle k, s\right\rangle$-privacy algorithm, it is not taken into account that population densities vary widely from place to place, and fixed $k$ and $s$ can not be adapted to all regions. For example, in the lively commercial street, there is a large population density, nevertheless, large privacy parameters will not significantly increase the degree of privacy protection, but will significantly reduce the availability of data. Conversely, in the rural areas and other areas with a relatively sparse population, privacy parameters seem too small. Due to sparse population and flat terrain, attackers can even easily find the user  in the user's current fuzzy private area quickly. And this exposes the user's location privacy. In the design of the $\left\langle k, s\right\rangle$ - privacy algorithm, there is no quantification of the actual degree of privacy protection, so that when the algorithm is used, the user can not understand how much private it is if he took different parameters. And also the privacy parameters are too large to many of the service providers, so it has basically lost the value of the data because these large data can not be effectively used. These problems make recommendation service providers lose the opportunity to improve the user experience and to give much privacy protection.

\subsection{$\left\langle r, h\right\rangle$-Privacy Location Privacy-Preserving Algorithm}
We improve the algorithm for the defects and shortcomings of $\left\langle k, s\right\rangle$-privacy, and put forward $\langle r, h\rangle$-privacy algorithm, and use Shannon information entropy to carry out theoretical evaluation. $\langle r, h\rangle$-privacy algorithm inherits the $\langle k, s\rangle$-privacy algorithm but we do not generate virtual nodes any more, but directly make the precise point blur into a circle, and the precise position may exist anywhere in the circle. In this ambiguous mode, the probability of the attacker  inferring the user's real position approaches $0$. In such a low or even $0$ probability case, the attacker also has a kind of violent attack to find the user. In this violent attacking mode, the $\langle r, h\rangle$-privacy algorithm is also able to adjust the radius of the circle by adjusting the radius of the local population, allowing the user to have enough time to leave without being found. It also assumes that an attacker can obtain all the information from the server, as in the $\langle k, s\rangle$-privacy algorithm. If a user wants to send a position to the server, a fuzzy one will be sent.
\par The $\langle r, h\rangle$-privacy algorithm is shown in Algorithm \ref{alg_rhA}. The algorithm first calculates the number of the given $POI_i$ history check-in points to determine whether the current POI is in a densely populated or sparsely populated place (line 2). Then calculate the radius of the virtual circle that needs to be blurred by the current $POI_i$ based on the sum $h_i$ of history check-in points (line 3). Finally, move the center point $POI_i$  to a new center $o^{'}$ with the vector of angle $\theta(\theta\in[0,2\pi])$, length $l (l\in(0,R))$ and we get the blurred circle (lines 4-6).
\\
...
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
  ...
  \begin{algorithmic}[1]
  \Require the position of $POI_i$
  \Ensure $o_i^{'}$, $R_i$
  \Function{\textit{CircleObfuscate(R,h)}}{the position of $POI_i$ }
  \State $h_i \leftarrow h(POI_i)$ \text{//the $POI_i^{'}s$ max number of history check-in}
  \State $R_i \leftarrow R(h_i)$ \text{// the radius are based on h}
  \State shift $POI_i$ to a random location $o^{'}$ in radius $R_i$
  \State $POI_i\leftarrow$ obfuscate $POI_i$ to a circle with center $o^{'}$ and radius $R_i$
  \State \Return{$o_i^{'}$ and $R_i$}
  \EndFunction
  \end{algorithmic}
  \end{algorithm}
\hrule
...
There are several ways to implement the virtual circle radius when calculating the total number of checkpoints as a parameter. And to be simple and effective, we choose the linear function to calculate:
\begin{equation}\label{equ:VirtualCR}
R_i=-\frac{R_{max}-R_{min}}{H_{max}}\cdot h_i+R_{max}
\end{equation}
where $R_{max}$ and $R_{min}$ respectively represent the upper and lower bounds of the virtual circle radius, and $H_{max}$ represents the maximum number of historical check-in numbers in all POIs($H_{max}=max\{h(POI_1),h(POI_2),\dots,h(POI_n)\}$). Last but not least , $R_{min}$ of virtual circle radius is set to prevent some hotspot POI from losing the privacy protection as the virtual circle radius is too small as the number of user checkouts increases.
\par In the $\langle r,h\rangle$-privacy algorithm, the total records $H_{max}$, the upper and lower bounds of the virtual circle radius $R_{max}$ and $R_{min}$, need to be further determined. $H_{max}$, such as Foursquare data ,can reach more than 10,000, and overview all the POI history records, only a small amount of POI can reach 10,000 or so, the vast majority of the POI history check number even less than 10. So it is more important to determine the upper bound of the radius than the lower bound.
\par The empirical value of $R_{max}$ is 200 meter. The maximum area of the virtual area is $S_{circle}=\pi\cdot R_{max}^{2} \approx 12500m^2 $ , only the $1/8$ of maximum privacy area in $\langle k, s\rangle $-privacy algorithm.
$\left\langle r, h\right\rangle$-Privacy will also face the situation of failure. For example, some POI history check numbers are only 1, and in this case the area of virtual circle reaches maximum. And here the center point $POI_i$  would move to a new center $o^{'}$ with the vector of angle $\theta(\theta\in[0,2\pi])$, length $l(l \in(0,R))$, however, there is a probability that there will be a small length $l$. Under this certain moment, attackers can quickly locate the precise position of POI and find the target user. Hence, in this case, we let length $l\in[R/2,R]$ to ensure that the virtual circle center will not be close to the real-time position.

\subsubsection{Theoretical analysis of $\left\langle r, h\right\rangle$-privacy}
We provide a attractive theoretical analysis way of GLP algorithm by using differential entropy \cite{CL2006} which supplies a quantitative indicator for the effect of blurring, i.e., the privacy guarantee.
%\begin{definition}[Support Set]\label{def:SupportSet}
...
\begin{theorem}[Distance distribution]\label{theorem:Dd}
...
f_Z(z)=F_Z^{'}(z)=(\frac{z^2}{R^2})^{'}=\frac{2z}{R^2}
\end{equation}
\end{proof}

\par When the algorithm is added to the POI recommendation system, the distance between any two POIs is required according to Eq.(\ref{eq:LW}) and Eq.(\ref{eq:PRLJ}) in Section \ref{sec:RelatedWork} when calculating the geographical location weight. In this paper, the distance $z_{i,j}=Dist(POI_i,POI_j)$ between two privacy POIs is defined as the distance between the two virtual circles as to facilitate the calculation. According to the Theorem \ref{theorem:Dd}, the probability density function of $z_{i,j}$ is:
\begin{equation}\label{equ:distance_2}
P(z)=\frac{2z}{R^2}
\end{equation}
\indent Then using the expectation $E(z_{i,j})$ of the probability density function of $z$ instead of the exact distance between the two POIs, we have
\begin{equation}\label{equ:distance_3}
E(z)=\int_{h-R}^{h+R}z\cdot\frac{2z}{R^2}\,dz=\frac{12h^2+4R^2}{3R}
\end{equation}
\indent And the POI was blurred into a virtual circle, resulting in the distance uncertainty between POIs. Hence the random variables $d_{i,j}$'s differential entropy can be calculated as
\begin{equation}\label{equ:distance}
\begin{aligned}
...
\section{FRP: friend relationship privacy-preserving algorithm}\label{sec:FRPPA}
In the social networking recommender system, each user's friends are potential attackers. They can derive the user's privacy check-in information with a large probability from the POI recommendation results. In Examples \ref{e.g.1} and \ref{e.g.2}, we have assumed that the attacker knows the recommendation algorithm of the recommender system, but the setting of each parameter in the algorithm is not achieved by attackers. For example, in Section \ref{sec:Background}, the predicted probability of the user for the POI is composed of three parts: user similarity weight, friend relationship weight, and location weight. These three weights are combined in a linear function, where the linear parameters are $1-\alpha-\beta$ , $\alpha$ and $\beta$, $0\leqslant \alpha+\beta\leqslant1$. In Example \ref{e.g.1}, the attacker Anna and the normal user Bran often go out and visit POIs together. Therefore, in the recommender system, Anna and Bran have higher user similarity $S_{A,B}^u$ , and they are friends, so their friend relationship weight $S_{A,B}^S$ is also high. In the top-K results returned by the recommender system to Anna, some abnormal points appear. These abnormal points are far away from the current location of hers, that is, the location weight $S_{A,B}^g$ is very low. Anna understands the recommendation algorithm so she infers that the POI in the recommended result must be the highest predicted check-in probability for her, and the result exists an abnormal point which has a very low location weight $S_{A,B}^g$. Hence, in the case of that three linear parameters $1-\alpha-\beta$ ,$\alpha$ and $\beta$ are constant, the user similarity weight $S_{A,B}^u$, and friend relationship weight $S_{A,B}^s$, must be very high, so user Anna can believe with a high probability that this anomaly is because of her best friend Bran who has a similar preference.
This section we will propose the FRP algorithm based on differential privacy for the friend relationship attacking  model in Section \ref{sec:Background}.
\subsection{Differentially private social relationship algorithm}
We add a differential privacy algorithm to the existing recommender system. So the algorithm has a certain degree of versatility. As long as it is used to calculate the weight between friends, the algorithm can be applied to the system.
\par The algorithm is based on social network and it can prevent the potential attack. The attacker may infer the target user's privacy information in a high probability if weights of the user similarity, friend relationship and location have a large difference in the POI recommendation system. There is a big difference in the special circumstances. Potential attackers can probably infer the target user's privacy information via this situation. Our algorithm can be simply understood that it can add enough noise to the weight of friend relationship, making the attacker not infer that his friend would have anything with the returned results from the system so that the attacker can not obtain any related privacy information.
\subsubsection{Friend relationship privacy-preserving algorithm}
...
\end{definition}
\begin{corollary}[$\epsilon$-differential privacy]\label{COR:DP}
 \cite{ZLZ2017} Let $F:D\rightarrow R^k$ be a score function, its sensitivity is $\Delta f$, then by Definition \ref{def:LapM}, $F(D)+Lap^k(\Delta F /\epsilon)$ satisfies $\epsilon$-differential privacy, where $Lap^k(\varphi)$ is a $k$-dimensional vector obtained from the Laplacian distribution with a standard variance of $\sqrt{2}\varphi$.
\end{corollary}
\par Let $Pr\{C_{B,j}=1|$top-K $POI\}$ represent that user A can infer the probability of user B checking in the $POI_j$ on the premise of system returning top-K POIs. The probability of inference $Pr\{C_{B,j}=1|$top-K $POI\}$ will not be particularly high if we add FRP in the recommender system, even though the attackers know the working process of the system. Although there are a number of basic definitions of privacy in the existing literatures, most definitions are too broad and have a drawback of no quantitative indicators, so in this article, we choose $\epsilon$ - differential privacy to build into FRP.
We describe it in Algorithm \ref{alg_frppA}. The algorithm first randomly deletes user $U_i$'s one friend relationship link from the database $D$, that is, they are no longer friends, and then generates a new neighbor database $D^{'}$ (lines 3-4). Second, we obtain the $L1$-vector norm of the two weight vectors, that is, the global sensitivity of the query function (line 5). The scale parameters of Laplacian noise can be obtained from the sensitivity of the query function (line 6). According to this, we can get the complete Laplace distribution and add Laplacian noise to the original friend relationship weight vector finally. We can see that the friend relationship weight become smoother and the relationship weight between the user $U_i$ and other users become more close, and it will not exist the situation that the friend relationship weights of some users are particularly high.
\\
\hrule
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
%\noindent\rule[0.25\baselineskip]{\textwidth}{1pt}
  ...
  \begin{algorithmic}[1]
  \Require the privacy parameter $\epsilon$
  \Ensure $SI_{i,k}^{'}$
 % \Function{\textit{Social Differential Privacy}}{the privacy parameter $\epsilon$}
  \For {each $u_i$ with user $u_k$'s $SI_{i,k}^{'}$ }
  \State database $D$ denotes all $u_i$’s relationship
  \State $D^’$ denotes delete one of $u_i$’s relationship, $D^{'}\leftarrow D-\{u_i,u_{random}\}$
  \State sensitivity $\Delta f\leftarrow max_{D,D^{'}}||SI_{i,k}(D)-SI_{i,k}(D^{'})||_1$
  \State scale factor $sf\leftarrow\Delta f /\epsilon$
  \State $\forall SI_{i,k}^{'}\leftarrow SI_{i,k}+Laplace(0,sf) $
  \EndFor
  \State \Return{$SI_{i,k}^{'}$}
 % \EndFunction
  \end{algorithmic}
  \end{algorithm}
\hrule
...
It is inferred that Algorithm \ref{alg_frppA} is a privacy recommendation one because it satisfies Definition \ref{def:PR}.  We still use the Shannon information entropy to evaluate the FRP algorithm because it mainly adopts wiping out the friend relationship weights with great differences for fuzzy implementation. There is a property that the bigger difference between users' friend relationship weights, the greater the information entropy, i.e., the greater uncertainty and the smaller the probability that the attacker derives the user's private information from the recommended POIs.
\begin{theorem}[Correctness]\label{theorem:correctness}
Friend relationship weight with differential privacy increases the information entropy of the friend relationship weight distribution, and improves the uncertainty of the weight distribution, that is $H(SI_i^{'})-H(SI_i)>0$ protects the user information privacy.
\end{theorem}
\begin{proof}
Let $SI_i$ be the weight distribution between user $u_i$ and other users. Let $SI_i^{'}$ be the privacy weight distribution of adding noise of Laplacian mechanism on $SI_i$. Then:
\begin{equation}\label{equ:correctness}
\begin{aligned}
H(SI_i^{'})-H(SI_i)&=H(SI_i+Lap(\frac{\Delta f}{\epsilon}))-H(SI_i)
...
2^{H(SI_i^{'})-H(SI_i)}=2^{1+\ln{(\frac{2\Delta f}{\epsilon})}}=4\times\frac{\Delta f}{\epsilon}
\end{equation}
where the result $4\times\Delta f /\epsilon$ is the support set of the differential entropy, so the result is always positive. Hence, $H(SI_i^{'})-H(SI_i)>0$.
\end{proof}

\section{Theoretical analysis of private POI recommendation algorithm}\label{sec:analysis}
We employ shannon entropy to measure the degree of privacy guarantees. First, to define privacy precisely, we need to determine a suitable model to describe the database. In many application domains, data can be represented as data tables, e.g., medical records, financial transaction records or employee records. The data table can be regarded simply as a relational database which normally consists of several data tables. A formal definition of database is as follows,
...
\end{itemize}
\end{definition}
\par We calculate the entropy of an individual record released by a database as
\begin{equation}
  H^{'}(X_{i}) = -\sum^{k}_{x=1}p^{'}_{ix}lg(p^{'}_{ix})
\end{equation}
where $H^{'}(X_{i})$ is the entropy of the $i$th record after a statistical release in the database and $H(X_{i})$ is associated with the $i$th record, i.e., $H(X_{i}) - H^{'}(X_{i})$ is the amount of information leakage of the record $r_{i}$  in the database. Similarly, we can transfer this method to calculate information loss of every record in the database. From this we can define privacy disclosure:
\begin{definition}[Privacy disclosure]\label{def:privacyleak}
\cite{CL2006} Let $H(X_{1}),H(X_{2}),\cdots,H(X_{i})$ be the entropies of each record in the database and $H^{'}(X_{1}),H^{'}(X_{2}),\cdots,H^{'}(X_{i})$ be the entropies of each record after a statistical release. Therefore, privacy disclosure can be defined as the maximum information loss of a record in the database. It is
\begin{equation}\label{equ:privacyleak}
  PrivacyDisclosure(P) = \max_{i \leq n}(H(X_{i}) - H^{'}(X_{i}))
\end{equation}
\end{definition}
...
\begin{lemma}[Entropy-chain rule]\label{lemma:entropy}
Let $X,Y,Z$ be three discrete random variables. If $X$, $Y$, and $Z$ are mutually independent and $M = M(X,Y,Z)$ is a one-to-one corresponding function, there will be $H(M) = H(X)+H(Y)+H(Z)$.
\end{lemma}
\begin{proof}
$M$ is a function about $M(X,Y,Z)$ and the random variables $X,Y,Z$ are independent of each other so there exists $H(X,Y,Z) = H(X)+H(Y|X)+H(Z|X,Y)\leq H(X)+H(Y)+H(Z)$ according to \cite{GRM2011}. Hence, it can be proved that $H(M) = H(X)+H(Y)+H(Z)$ iff $M(X,Y,Z)$ is also a function about $M$.
\end{proof}
\begin{theorem}[Balance formula]\label{theorem:BF}
Under the assumption that the three parameters are pairwise independent, the entropy of the POI probability distribution obtained by the POI privacy recommendation algorithm is greater than the common recommendation algorithm.
\end{theorem}
\begin{proof}
It is proved that the information entropy of location and friend relationship increased after adding privacy-preserving algorithm in Sections \ref{sec:Location PPA} and \ref{sec:FRPPA}, where $H(z)=H(2z/R^{2})>0$ and $H(SI_i^{'})-H(SI_i)>0$, $z$ represents the probability distribution of the distance between a POI to another fuzzy circle of a POI, $SI$ represents the probability distribution of the friend relationship weight between the user and all other users. Based on the above two formulas and Lemma \ref{lemma:entropy}, the balance formula is given by
\begin{equation}\label{equ:BF}
\begin{aligned}
\Delta H&=H(S_{i,j}^{'})-H(S_{i,j})
\\&=H(S_{i,j}^{u}+S_{i,j}^{'s}+S_{i,j}^{'g})-H(S_{i,j}^{u}+S_{i,j}^{s}+S_{i,j}^{g})
\\&=H(S_{i,j}^{'s}+S_{i,j}^{'g})-H(S_{i,j}^{s}+S_{i,j}^{g})
\\&=H(\frac{2z}{R^{2}})+H(Lap(\frac{\Delta f}{\epsilon})) > 0
\end{aligned}
\end{equation}
\end{proof}
\par In the Eq.(\ref{equ:BF}), $\Delta H$ is the incremental information entropy after adding the privacy-preserving algorithm, and it represents the degree of privacy guarantees. And differential privacy parameter $\epsilon$ which is from the friend relationship weight and the query function sensitivity $\Delta f$ can be set by users before to ensure a reasonable privacy guarantee. On the location weight, in addition to the system default radius of the virtual circle, users can also customize the radius, since the parameter $R$ is also a reasonable control of privacy guarantee for them.
...
In Fig.\,\ref{fig:Recall5Precision5}, the best performance of {\color{red}normal algorithm} is achieved when $\alpha$ and $\beta$ are both equal to $0.1$. The result is probably due to the fact that the user similarity factor plays a decisive role among the three factors. The higher weight of user similarity  usually makes contribution to the higher performance of the recommendation algorithm. At the same time, the factors of friendship  and geographical location are also non-negligible for the recommendation algorithm since they at least take an account for $20\%$.
...
we know that the location factor has the minimum effect on the performance of recommendation  among all the weight combinations. %Although the location weight and the friend relationship weight both account for about $10\%$,
...
{\color{red} connections to the above paragragh ???}In the GLP algorithm, when each POI is blurred into a virtual circle, the radius of the virtual circle is determined by the historic check-in numbers. As shown in Fig.\,\ref{fig:NoPOI}, it is a logarithmic photo, i.e., the user number of $x$-axis and the POI check-in number of $y$-axis are log-processed, and it can fit into an approximate straight line, which indicates that the original data are better fitted to the exponential mechanism. The check-in number of POIs starts at about 1000 and then drops rapidly until 1. And we also know that a majority of POI check-in numbers are concentrated in [1, 20], that is, the check-in number which is less than 100 accounted for nearly $99\%$ of the total. Hence, the virtual circle radius of most POIs is very close to the maximum. Due to this fact, we let $H_{max}=100$, for 43186 POIs, almost 500 of the hottest POIs' virtual circle radius will achieve the minimum $R_{min}$, and $18\%$ of POIs' radius will achieve the maximum $R_{max}$.
...
%
%    \bibitem{TH2005}
%     T. Hofmann, D. Hartmann.(2005). Collaborative Filtering with Privacy via Factor Analysis. Proceedings of the 2005 ACM Symposium on Applied Computing. In (pp.791–795).
%
%    \bibitem{SH2009}
...
%
%    \bibitem{JC2002}
%    J. Canny. Collaborative filtering with privacy.(2002). Security and Privacy. Proceedings. 2002 IEEE Symposium on. In IEEE'02 (pp.45–57).
%
%    \bibitem{SB2007}
...
%
%    \bibitem{MY2011}
%    M. Ye, P. Yin, W.-C. Lee et al.(2011). Exploiting geographical influence for collaborative point-of-interest recommendation. Proceedings of the 34th international ACM SIGIR conference on Research and development in Information Retrieval. In ACM'11 (pp.325–334).
%
%    \bibitem{DK2003}
...
%
%    \bibitem{CL2006}
%    V.Chirayath, L.Longpré.(2006). Measuring privacy loss in statistical databases. International Workshop on Descriptional Complexity of Formal Systems - Dcfs 2006, Las Cruces, New Mexico, Usa, June 21 - 23, 2006. Proceedings (pp.16-25). DBLP.
%
%    \bibitem{AKU2014}
